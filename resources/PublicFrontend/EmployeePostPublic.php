        ...
        while ($employeePost-> have_posts()) {
            ...
            <div class="company-employee-container">
                    <div class="company-employee-box"  id="employee-box">
                        ...
                        <h3><?php echo esc_html(get_post_meta($post->ID, 'first_name', true)) ?><span><?php echo esc_html(get_post_meta($post->ID, 'last_name', true)) ?></span></h3>
                        <p><?php echo esc_html(get_post_meta($post->ID, 'position', true)) ?></p>
                        <div class="company-employee-bottom">
                            ...
                            <div class="company-employee-bottom-popup" id="employee-bottom-popup">
                                <p class="company-employee-description"><?php echo esc_html(get_post_meta($post->ID, 'short_description', true)) ?></p>
                                <div class="company-employee-social-link">
                                    <p><a href="<?php echo esc_html(get_post_meta($post->ID, 'facebook_link', true)) ?>"><span class="dashicons dashicons-facebook-alt"></span>facebook</a></p>
                                    <p><a href="<?php echo esc_html(get_post_meta($post->ID, 'linkedIn_link', true)) ?>"><span class="dashicons dashicons-linkedin"></span>linkedIn</a></p>
                                    <p><a href="<?php echo esc_html(get_post_meta($post->ID, 'github_link', true)) ?>"><span  class="dashicons"><img src="<?php echo esc_url(plugins_url('assets/GitHub-32px.png', dirname(__DIR__))) ?>"></span>github</a></p>
                                    <p><a href="<?php echo esc_html(get_post_meta($post->ID, 'xing_link', true)) ?>"><span class="dashicons  dashicons-xing"></span>xing</a></p>
                                </div>
